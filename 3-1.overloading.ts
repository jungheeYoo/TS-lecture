...
함수 오버로딩을 사용하면 동일한 호출 서명(call signature)을 가진 여러 함수를 정의할 수 있습니다. 이것은 함수의 이름이 같지만 매개변수의 타입 또는 개수가 다른 경우에 특히 유용합니다. 프로그래밍 언어가 함수 호출을 구별하기 위해 매개변수의 타입과 개수를 고려하기 때문에, 서로 다른 매개변수를 가진 여러 함수를 정의함으로써 동일한 이름으로 여러 기능을 수행할 수 있습니다. 이것은 코드를 더욱 명확하고 읽기 쉽게 만들어줍니다.
*/
